...
% ||(x,y)_i-(x,y)_j||∞ ≤ d_max
% ||(x,y)_i-(x,y)_j||∞ ≥ d_min
d_max = 50; % maximum distance between vehicles - [m]
...
for i=1:N
    ...
    vehicle{i}.cg.add_vehicle_cnstr('position',[x_max,50],'thrust',T_max,'speed',[Vx,Vy]);
    for j=1:N
...
for t=1:NT
    for i=1:N
        if vehicle{i}.color == colors(round)
            ...
            if(i==1)
                obseen = ob1.seen_obstacles(pos(1:2), r_vision, oblist);
            end             
            if(i==2)
                obseen = ob1.seen_obstacles(pos(1:2), r_vision, oblist);
            end
