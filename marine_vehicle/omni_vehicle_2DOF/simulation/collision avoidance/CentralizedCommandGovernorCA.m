classdef CentralizedCommandGovernorCA < CommandGovernor
    ...
    methods
        function obj = CentralizedCommandGovernorCA(Phi,G,Hc,L,T,gi,U,hi,Psi,k0,solver)
        ...
        function [g, ris] = compute_cmd(obj,x,r,ve1,ve2,ve3,ve4)
            ...
            sdpvar rr1 rr2;
            mu = 1000;
            [aris1,bris1] = find_hyperplane(ve1,ve2,ve3,ve4,x(1:2)); 
            [aris2,bris2] = find_hyperplane(ve1,ve2,ve3,ve4,x(7:8)); 
...
%                 cnstr = [cnstr obj.T*(obj.Hc*xk+obj.L*w) <= obj.gi];
                ...
                cnstr = [cnstr aris1'*xk(1:2) <= (bris1-rr1)];
                cnstr = [cnstr aris2'*xk(7:8) <= (bris2-rr2)];
...
%                     cnstr = [cnstr (obj.U((i-1)*4+4,:)*(obj.Hc*xk+obj.L*w)) >=  obj.hi((i-1)*4+4)-mu*b((k-1)*size(obj.U,1)+(i-1)*4+4)];
            ...
            cnstr = [cnstr aris1'*w(1:2) <= (bris1)];
            cnstr = [cnstr aris2'*w(3:4) <= (bris2)];
