classdef DistribuitedCommandGovernor < CommandGovernor
    ...
    %  
    %   dcg = DISTRIBUITEDCOMMANDGOVERNOR(Phi,G,Hc,L,T,gi,U,hi,Psi,k0) creates an instance 
    %      of Distributed Command Governor with the closed-loop matrices Phi, G, Hc, L.
    %      Convex constrains are expressed as T*c <= gi. OR-ed (like collition avoidance)
    %      constraints, which are not convex, are expressed as U*c <= hi-μ*b with b binary
    %      variables. Psi is the reference weight matrix, k0 is the prediction horizon.
    %  
    %   dcg = DISTRIBUITEDCOMMANDGOVERNOR(Phi,G,Hc,L,T,gi,U,hi,Psi,k0, solver) creates an 
    %      instance of Distributed Command Governor with the closed-loop matrices Phi, G,
    %      Hc, L. Convex constrains are expressed as T*c <= gi. OR-ed (like collition avoidance)
    %      constraints, which are not convex, are expressed as U*c <= hi-μ*b with b binary
    %      variables. Psi is the reference weight matrix, k0 is the prediction horizon.
    %      Default solver is 'bmibnb'.
    % 
    ...
    methods
        ...
        function [g, ris, hype, alpha] = compute_cmd(obj,x,r,g_n,cloud_points,hyp, path_x, path_y, alpha_neig)
            ...
            %   [g,ris] = COMPUTE_CMD(x,r,g_n) Calculate the nearest reference g to r start from initial
            %   conditions x and g_n reference for the other systems. Output of optimization problem 
            %   solving is returned in ris.
...
%                 
        ...
        function [a_ris, b_ris, diagnos] = find_hyperplane_cg(~, y, d_min, g, cloud_points, hyp)
            ...
            for i = 1:length(cloud_points(1, :))
                V = [V,(a'*cloud_points(:, i))>=(b+d_min+0.5)];
            end
